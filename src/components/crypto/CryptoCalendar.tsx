...
export default function CryptoCalendar() {
  ...
  return (
    <div className="w-full mb-8 bg-slate-900 rounded-3xl p-6 border border-slate-800 shadow-sm">
      <div className="flex items-center gap-3 mb-4">
        <div className="p-2 bg-indigo-900/50 rounded-xl">
          <Calendar className="w-5 h-5 text-indigo-400" />
        </div>
        <h2 className="text-xl font-bold tracking-tight text-white">Macro Calendar (7 Hari)</h2>
      </div>
          ...
          return (
            <div 
              ...
              className={`snap-center shrink-0 w-[120px] rounded-2xl p-4 cursor-pointer transition-all duration-300 border 
                ${isLatest ? 'bg-indigo-600 text-white border-indigo-500 shadow-lg shadow-indigo-600/30 -translate-y-1 hover:-translate-y-2' : 'bg-slate-900/50 text-slate-300 border-slate-800 hover:bg-slate-800'}
              `}
            >
              <div className={`text-xs font-semibold uppercase tracking-wider mb-1 ${isLatest ? 'text-indigo-200' : 'text-slate-500 text-slate-400'}`}>
                {dayName}
              </div>
              <div className="text-3xl font-black mb-1">
                {dayNum}
              </div>
              <div className={`text-sm font-medium ${isLatest ? 'text-indigo-100' : 'text-slate-500'}`}>
                {monthStr}
              </div>
            </div>
          );
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-[500px] rounded-3xl p-0 overflow-hidden bg-slate-50 bg-slate-950">
          {selectedReport && (
            <>
              ...
              <div className="p-6 bg-slate-900">
                <div className="prose prose-sm prose-invert max-w-none prose-p:leading-relaxed prose-headings:font-bold prose-a:text-indigo-500">
                  ...
                  <div className="bg-slate-800/50 p-4 rounded-2xl text-sm text-slate-300 whitespace-pre-wrap leading-relaxed border border-slate-800">
