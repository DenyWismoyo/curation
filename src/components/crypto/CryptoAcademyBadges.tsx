          ...
          {stats.badges && stats.badges.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {stats.badges.map((badge, idx) => {
                ...
                return (
                  <div key={idx} className="flex flex-col items-center justify-center p-4 bg-slate-800/50 rounded-xl border border-slate-700/50 hover:bg-slate-800 transition-colors group">
                    <div className="p-3 bg-slate-900 rounded-full mb-3 ring-1 ring-white/10 group-hover:ring-white/20 group-hover:scale-110 transition-all">
                      <Icon className={`w-8 h-8 ${config?.color || "text-purple-500"}`} />
                    </div>
                    <p className="font-bold text-sm text-white text-center mb-1">{badge}</p>
