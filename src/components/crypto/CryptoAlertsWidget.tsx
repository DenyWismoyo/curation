  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button variant="outline" size="icon" className="relative rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors w-10 h-10 border-slate-200 dark:border-slate-800">
      ...
      <SheetContent side="right" className="w-full sm:w-[400px] p-0 flex flex-col bg-slate-50 dark:bg-slate-950 border-l border-slate-200 dark:border-slate-800 shadow-2xl">
        <SheetHeader className="px-6 py-5 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 relative z-10 shadow-sm">
           ...
           <ScrollArea className="h-full px-6 py-4">
             {loading ? (
               ...
               <div className="flex flex-col items-center justify-center h-full text-center text-slate-400 mt-20">
                 ...
                 <h4 className="font-bold text-slate-700 dark:text-slate-300 mb-1">Kosong</h4>
                 <p className="text-sm">Belum ada riwayat notifikasi scalping.</p>
               </div>
             ) : (
               <div className="space-y-4 pb-10">
                 {alerts.map((alert) => (
                   <div key={alert.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
                     <div className="absolute top-0 left-0 w-1 h-full bg-rose-500"></div>
                     <div className="flex justify-between items-start mb-2 pl-2">
                       <Badge className="bg-rose-100 text-rose-700 border-rose-200 hover:bg-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800 uppercase text-[10px] tracking-wider font-bold">
                     ...
                     <p className="text-sm text-slate-600 dark:text-slate-400 pl-2 whitespace-pre-wrap leading-relaxed line-clamp-3 group-hover:line-clamp-none transition-all duration-300">
                       {alert.body}
                     </p>
                     
                     <div className="mt-4 pl-2 flex items-center justify-between">
                        <div className="flex gap-3 text-xs font-medium">
                           <span className="text-emerald-600 dark:text-emerald-400">TP: {alert.targetPrice || '-'}</span>
                           <span className="text-rose-600 dark:text-rose-400">SL: {alert.stopLossPrice || '-'}</span>
                        </div>
                        <Link href={`/crypto-report/${alert.symbol.replace('USDT', '')}`} onClick={() => setIsOpen(false)}>
                           <Button variant="ghost" size="sm" className="h-7 text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 text-xs px-2">
