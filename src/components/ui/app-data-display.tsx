...
const listVariantMap = {
  ...
  striped: 'flex flex-col rounded-2xl overflow-hidden ring-1 ring-slate-200/60 shadow-sm',
...
export function AppKeyValueItem({ label, value, icon, className, listVariant = 'card', index = 0 }: AppKeyValueItemProps) {
  ...
  if (listVariant === 'card') {
    return (
      <div className={cn("bg-slate-50 p-4 sm:p-5 rounded-2xl ring-1 ring-slate-100 flex flex-col justify-center", className)}>
        <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-2">
          {icon} {label}
        </p>
        <div className="text-sm font-bold text-slate-800 break-words whitespace-pre-wrap leading-relaxed">
          {value}
        </div>
      </div>
    );
  ...
  if (listVariant === 'striped') {
    return (
      <div className={cn(
        ...
        "bg-white border-b border-slate-100 last:border-b-0 hover:bg-slate-50/50"
      )}>
        ...
        <span className="text-sm font-bold text-slate-900 sm:w-2/3 sm:text-right break-words whitespace-pre-wrap">
          {value}
        </span>
      </div>
    );
  ...
  // divided
  return (
    <div className={cn("flex flex-col sm:flex-row sm:items-center justify-between py-3 border-b border-slate-100 last:border-b-0", className)}>
      ...
      <span className="text-sm font-bold text-slate-900 text-left sm:text-right break-words max-w-full sm:max-w-sm lg:max-w-md">
        {value}
      </span>
    </div>
  );
...
export function AppEmptyState({
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center p-8 text-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/50", minHeight, className)}>
      {icon && (
        <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-sm ring-1 ring-slate-200/60 mb-5 text-slate-400">
          {icon}
        </div>
      )}
      <h3 className="text-lg font-black text-slate-900 mb-2">{title}</h3>
      <p className="text-sm font-medium text-slate-500 max-w-sm mx-auto leading-relaxed mb-6">
        {description}
      </p>
      {action && (
        <div>{action}</div>
...
const infoCardVariantMap = {
  default: 'bg-white ring-slate-200 text-slate-900 icon-slate-500',
  primary: 'bg-indigo-50 ring-indigo-200 text-indigo-900 icon-indigo-600',
  success: 'bg-emerald-50 ring-emerald-200 text-emerald-900 icon-emerald-600',
  warning: 'bg-amber-50 ring-amber-200 text-amber-900 icon-amber-600',
  danger: 'bg-rose-50 ring-rose-200 text-rose-900 icon-rose-600',
};
  ...
  return (
    <div className={cn("p-4 sm:p-5 rounded-2xl ring-1 shadow-sm flex flex-col justify-between transition-all hover:shadow-md", bgClass, ringClass, className)}>
      <div className="flex items-start justify-between mb-2">
        ...
        {icon && (
          <div className={cn("w-8 h-8 rounded-lg bg-white/60 flex items-center justify-center shrink-0 shadow-sm", iconClass)}>
            {icon}
          </div>
        )}
      </div>
