    ...
    return (
      <div ref={ref} className="bg-white flex flex-col font-sans text-slate-900 w-[1024px]">
        ...
        <div style={pageStyle} className="flex flex-col relative">
          ...
          {isValidated && (
            <div className="absolute top-[80px] right-[80px] border-[6px] border-emerald-500 text-emerald-500 px-6 py-3 rotate-12 opacity-90 z-50 rounded-2xl flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm">
              <span className="text-[28px] font-black tracking-[0.2em] uppercase leading-none mb-1">FIELD</span>
              <span className="text-[28px] font-black tracking-[0.2em] uppercase leading-none">VALIDATED</span>
            </div>
          )}
          ...
          <div className="flex justify-between items-start mb-16">
            ...
            <div className={`border px-4 py-2 ${isValidated ? 'border-emerald-500 bg-emerald-50' : 'border-slate-300'}`}>
              <p className={`text-[10px] font-black tracking-[0.3em] uppercase ${isValidated ? 'text-emerald-700' : 'text-slate-900'}`}>
                {isValidated ? 'Validated Report' : 'Assessment Report'}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-12 gap-10 mb-8">
            <div className="col-span-4 flex flex-col">
              <div className={`border p-8 flex flex-col items-center justify-center text-center aspect-square mb-6 ${isValidated ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>
                <p className="text-[12px] font-bold tracking-[0.2em] text-slate-500 uppercase mb-4">Readiness Score</p>
                <div className={`text-[100px] font-black leading-none tracking-tighter mb-4 ${isValidated ? 'text-emerald-900' : 'text-slate-900'}`}>{Math.min(aiResult.totalScore, 100)}</div>
                <div className={`h-1 w-12 mb-4 ${isValidated ? 'bg-emerald-600' : 'bg-indigo-600'}`}></div>
                <p className={`text-[12px] font-black uppercase tracking-widest ${isValidated ? 'text-emerald-700' : 'text-slate-700'}`}>{aiResult.readinessLevel}</p>
              </div>
            </div>
        ...
        {/* HALAMAN 5: RECOMMENDED PATHWAY & CATATAN KURATOR */}
        <div style={{...pageStyle, backgroundColor: '#0f172a'}} className="text-white flex flex-col justify-center items-center text-center">
          <div className={`absolute top-0 left-0 w-full h-2 ${isValidated ? 'bg-emerald-500' : 'bg-indigo-600'}`}></div>
          <div className="relative z-10 w-full max-w-4xl flex flex-col items-center justify-center">
            <div className={`w-20 h-20 border rounded-2xl flex items-center justify-center mb-12 ${isValidated ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-indigo-500/10 border-indigo-500/30'}`}>
              <Compass className={`w-10 h-10 ${isValidated ? 'text-emerald-400' : 'text-indigo-400'}`} />
            </div>
            
            <p className={`text-[14px] font-bold tracking-[0.4em] uppercase mb-8 border-b pb-4 inline-block ${isValidated ? 'text-emerald-400 border-emerald-500/30' : 'text-indigo-400 border-indigo-500/30'}`}>
              {isValidated ? 'Validated Pathway' : 'Final Recommendation & Pathway'}
            </p>
            ...
            {isValidated && (
              <div className="w-full max-w-3xl bg-emerald-950/30 border border-emerald-500/30 p-6 mt-4 rounded-2xl text-left">
                <h4 className="text-[12px] font-black uppercase tracking-widest text-emerald-400 mb-3 flex items-center gap-2">
                  <ShieldCheck size={16}/> Field Check Notes / Catatan Kurator
                </h4>
                <p className="text-[14px] text-emerald-100/80 font-medium leading-relaxed whitespace-pre-wrap">
                  {aiResult.curatorNotes}
                </p>
              </div>
            )}

            <div className="mt-16 flex items-center gap-4 border border-slate-700 rounded-full px-6 py-3">
              <span className={`w-2 h-2 rounded-full animate-pulse ${isValidated ? 'bg-emerald-500' : 'bg-indigo-500'}`}></span>
