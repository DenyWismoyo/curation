...
export function CurationDashboard({ trackType, formData, aiResult, onRestart }: Props) {
  ...
  const [expandedSection, setExpandedSection] = useState<string | null>('rec-0'); // Default buka rekomendasi pertama
      ...
      <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8">
        ...
        <div className="space-y-6 sm:space-y-8 bg-slate-50 p-2 sm:p-6 lg:p-8 rounded-2xl relative overflow-hidden">
          <div className="relative z-10 space-y-6 sm:space-y-8">
            ...
            <div className={`relative overflow-hidden rounded-[2rem] sm:rounded-[2.5rem] p-8 sm:p-12 lg:p-16 text-white shadow-xl flex flex-col items-center sm:items-start text-center sm:text-left ${isHighTier ? 'bg-[#0f3d32]' : 'bg-slate-900'}`}>
              ...
              <div className="relative z-10 w-full flex flex-col lg:flex-row justify-between items-center gap-8 lg:gap-12">
                ...
                {/* Eksekutif summary kini menjadi item pertama dari rekomendasi (jika ada), atau gunakan dummy jika format lama */}
                <div className="lg:max-w-md w-full bg-white/10 backdrop-blur-xl ring-1 ring-white/20 p-6 rounded-3xl shadow-2xl">
                  <h3 className="text-indigo-200 font-black uppercase tracking-widest text-xs mb-3 flex items-center gap-2"><Zap className="h-4 w-4"/> Executive Summary</h3>
                  <p className="text-slate-100 leading-relaxed text-sm font-medium line-clamp-5">
                    {aiResult.recommendations?.[0]?.content || "Analisis strategis selesai dievaluasi berdasarkan metrik industri."}
                  </p>
                </div>
              </div>
            </div>

{/* PERUBAHAN 2: Tampilan Radar Chart & Legenda di bawahnya */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="bg-white p-6 sm:p-8 rounded-[2rem] ring-1 ring-slate-200 shadow-sm flex flex-col items-center">
                <h4 className="font-black text-slate-900 mb-2 text-center text-lg tracking-tight">Dimensi Kinerja</h4>
                ...
                {/* Area Keterangan Teks (Legenda) di Bawah Grafik */}
                <div className="w-full mt-4 grid grid-cols-2 gap-x-3 gap-y-3 pt-4 border-t border-slate-100">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="flex flex-col gap-3">
                  {aiResult.recommendations?.map((rec, idx) => (
                    <InsightAccordion 
                      ...
                      id={`rec-${idx}`} 
                      title={rec.title} 
                      icon={Target} // Icon generik yang elegan
                      content={rec.content} 
                    />
                  ))}
                </div>
              ...
              <div className="lg:col-span-1 flex flex-col gap-4 sm:gap-6">
                <div className={`p-6 sm:p-8 rounded-[2rem] text-center ring-1 shadow-sm ${isHighTier ? 'bg-emerald-50 ring-emerald-200/60 text-emerald-900' : 'bg-indigo-50 ring-indigo-200/60 text-indigo-900'}`}>
                  ...
                  <h4 className="text-xl sm:text-2xl font-black leading-tight tracking-tight text-balance">{aiResult.incubationRoute || "Rute Umum"}</h4>
                </div>
