// src/app/components/shared/OmniAiWidget.tsx
'use client';

import React, { useState, useRef, useEffect, useMemo } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { X, Send, Sparkles, User as UserIcon, AlertCircle, Trash2, Zap } from 'lucide-react';
...
interface OmniAiWidgetProps {
  isOpen: boolean;
  onClose: () => void;
}

export function OmniAiWidget({ isOpen, onClose }: OmniAiWidgetProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    if (savedChat) {
      ...
      setMessages([{
        id: 'welcome-msg',
        sender: 'ai',
        text: 'Selamat datang di **Omnifit**! Saya **Omni AI**, asisten navigator cerdas Anda. Ada yang bisa saya bantu terkait informasi platform atau panduan navigasi halaman ini?',
      }]);
    }
  ...
  const handleClearChat = () => {
    const resetMsg: Message[] = [{
      id: 'welcome-msg',
      sender: 'ai',
      text: 'Memori sesi ini telah dibersihkan. Apakah ada fitur platform **Omnifit** yang ingin Anda jelajahi bersama **Omni AI**?',
    }];
  ...
  const handleSendMessage = async (textOverride?: string) => {
    ...
    try {
      ...
      const chatWithOmniAi = httpsCallable(functions, 'chatWithOmniAi');
      
      const response = await chatWithOmniAi({
         message: userText,
      ...
      setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), sender: 'system', text: `Kalibrasi ulang: ${error.message || 'Gagal terhubung.'}` }]);
    } finally {
      setIsTyping(false);
    }
  };
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          onDragEnd={(e, info) => {
            // Tutup jika digeser ke kanan cukup jauh / cepat
          ...
          <div className="bg-white/90 border-t border-slate-100 shrink-0 relative z-10 flex flex-col p-4 pointer-events-auto">
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="relative flex items-center group px-1">
              ...
              <Button
                ...
                size="icon"
                className="absolute right-2.5 w-9 h-9 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white shadow-md shadow-indigo-600/20 transition-transform active:scale-95 disabled:opacity-50"
              >
