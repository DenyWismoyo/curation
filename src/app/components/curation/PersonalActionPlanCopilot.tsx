...
import { AIResult, CurationFormData, AiPromptConfig } from '@/types/curation';
    ...
    return (
      <div className="bg-white ring-1 ring-slate-200 p-6 rounded-[2rem] shadow-sm">
        ...
        <div className="text-center p-6 bg-slate-50 rounded-2xl border border-slate-100">
          ...
          <h4 className="font-bold text-slate-800 mb-2">Buat Rencana Aksi Spesifik</h4>
  ...
  return (
    <div className="bg-white ring-1 ring-slate-200 p-6 rounded-[2rem] shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-slate-900 font-black uppercase tracking-widest text-xs flex items-center gap-2 mb-1">
            <Sparkles size={16} className="text-indigo-500" /> Action Plan Personal
          </h3>
          <p className="text-xs text-slate-500">Langkah-langkah yang direkomendasikan secara khusus untuk Anda.</p>
        </div>
        <button
          onClick={handleGenerate}
          disabled={isGenerating}
      ...
      <div className="space-y-4">
        {actionPlan.map((task: any, index: number) => {
          ...
          return (
            <div key={task.id || index} className="border border-slate-200 rounded-2xl overflow-hidden transition-all bg-white">
              {/* Task Header */}
              <div 
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <h4 className="font-bold text-slate-800 text-sm leading-tight">{task.task}</h4>
              ...
              {isExpanded && (
                <div className="bg-slate-50 p-4 border-t border-slate-100 space-y-4">
                  {/* Tips & Tricks */}
                  {task.contextualTip && (
                    <div className="flex items-start gap-2 text-amber-700 bg-amber-50/50 p-3 rounded-xl border border-amber-100/50">
                  ...
                  {task.searchKeyword && (
                    <div className="pt-2">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-indigo-50 text-indigo-600 text-[10px] font-bold">
                        <Sparkles size={10} /> 
