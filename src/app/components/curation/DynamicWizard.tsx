...
export function DynamicWizard({ template, onComplete, onBack }: DynamicWizardProps) {
  ...
  // STATE MENGGUNAKAN LOCAL STEPS (Agar bisa diinjeksi step baru oleh AI)
  ...
  const [hasBeenInterrogated, setHasBeenInterrogated] = useState(false);
  ...
  const handleClearForm = () => {
    if (window.confirm('Apakah Anda yakin ingin mengosongkan semua isian form?')) {
      ...
      setStep(1);
      setLocalSteps(template.steps); // Reset steps ke awal
      setHasBeenInterrogated(false);
  ...
  // LOGIKA NAVIGASI & INJEKSI AI ASINKRON
      ...
      if (!hasBeenInterrogated) {
        ...
        try {
          ...
          if (data.fields && data.fields.length > 0) {
            const aiStep: FormStep = {
              stepNumber: totalSteps + 1,
              title: "Klarifikasi Otak AI",
              description: "Berdasarkan analisis algoritma kami terhadap pola jawaban Anda, mohon elaborasi poin berikut secara objektif.",
              icon: "BrainCircuit",
              fields: data.fields
            };
            
            setLocalSteps(prev => [...prev, aiStep]);
            setHasBeenInterrogated(true);
          ...
          toast.error("Gagal menyinkronkan dengan AI, beralih ke tahap akhir.");
          setIsReviewMode(true);
        } finally {
      ...
      <header className="bg-white border-b border-slate-200 shrink-0 relative z-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
          ...
          <button 
            ...
            className="flex items-center gap-1.5 text-slate-500 hover:text-indigo-600 transition-colors w-20 sm:w-28"
          >
            ...
            {!isAnalyzingAI && currentStepData && (
              ...
              >
                {step > template.steps.length ? (
                   <BrainIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-indigo-600" />
                ) : (
          ...
          <div className="flex items-center justify-end gap-3 w-20 sm:w-28">
            ...
            <button 
              ...
              className="text-[10px] font-bold text-rose-500 hover:bg-rose-50 px-2 py-1.5 rounded-lg transition-colors flex items-center gap-1"
            ...
            {isAnalyzingAI ? (
              ...
              <motion.div key="ai-loading" variants={formVariants} initial="enter" animate="center" exit="exit" className="flex flex-col items-center justify-center py-20 text-center space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl font-black text-slate-900">Menyinkronkan Argumen...</h3>
                  <p className="text-slate-500 font-medium max-w-md mx-auto leading-relaxed">
                    Otak AI kami sedang membaca pola jawaban Anda dan memformulasikan matriks klarifikasi lanjutan secara real-time.
                  </p>
                </div>
              </motion.div>
            ) : (
              // LAYAR PERTANYAAN STANDAR / KLARIFIKASI AI
              <motion.div key={`step-${step}`} variants={formVariants} initial="enter" animate="center" exit="exit">
                
                {/* Lencana Peringatan Jika Ini Adalah Step Injeksi AI */}
                {step > template.steps.length && (
                  <div className="flex justify-center mb-6">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 rounded-full text-[11px] font-black uppercase tracking-widest ring-1 ring-indigo-200/50 shadow-sm animate-pulse">
                      <Sparkles size={14} /> Tahap Validasi AI
                    </div>
                  </div>
                )}
                
                {/* Loop Rendering Form Fields (Murni tanpa deskripsi step) */}
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
              ...
              >
                {step < totalSteps ? (
                  ...
                  <>Analisis Data <DocExportIcon size={18} className="group-hover:scale-110 transition-transform" /></>
                )}
              </Button>
            </div>
