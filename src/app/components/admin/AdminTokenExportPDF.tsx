  ...
  return (
    <>
      <Button 
        ...
        className="bg-rose-600 hover:bg-rose-700 text-white gap-2 font-bold rounded-xl h-9 px-3 transition-all active:scale-95 shadow-sm"
      >
        ...
        <div ref={pdfRef}>
          ...
          <div className="pdf-page flex flex-col justify-start font-sans text-slate-800" style={pageStyle}>
            ...
            <div className="relative z-10">
              <h1 className="text-4xl font-black text-slate-900 border-b-[4px] border-indigo-600 pb-4 mb-10">
                Buku Panduan & Daftar Token Akses AI
              </h1>
              
              <div className="bg-slate-50 border border-slate-200 p-8 rounded-2xl flex justify-between font-bold text-slate-700 text-lg mb-12">
                 <div>
                   <div className="text-2xl text-slate-900 mb-2">Entitas: {batch.corporateName}</div>
                   <div>Tipe Mesin AI: <span className="text-indigo-600">AI {batch.modelType?.toUpperCase() || 'FLASH'}</span></div>
                 </div>
                 <div className="text-right space-y-2">
                   <div>ID Prefix: <span className="text-indigo-600">{batch.id}</span></div>
                   <div>Total Alokasi: {batch.totalTokens} Token</div>
                   <div className="font-medium text-slate-500">Dicetak pada: {printDate} WIB</div>
                 </div>
              </div>

              <h2 className="text-2xl font-bold text-indigo-700 mb-6 border-b-2 border-slate-200 pb-3">1. Pengenalan Token Akses</h2>
              ...
              <h2 className="text-2xl font-bold text-indigo-700 mt-6 mb-6 border-b-2 border-slate-200 pb-3">2. Langkah-Langkah Pengisian Form</h2>
            ...
            <div className="absolute bottom-12 left-[80px] right-[80px] text-center text-sm font-bold text-slate-400 border-t-2 border-slate-100 pt-6">
          ...
          {tokenChunks.map((chunk, pageIndex) => (
            <div key={pageIndex} className="pdf-page flex flex-col justify-start font-sans text-slate-800" style={pageStyle}>
              <h2 className="text-2xl font-bold text-indigo-700 mb-8 border-b-2 border-slate-200 pb-3">
              ...
              <table className="w-full border-collapse border border-slate-300 text-lg">
                 <thead>
                   <tr className="bg-slate-100 text-slate-800">
                     <th className="border border-slate-300 p-4 text-center w-[8%]">No</th>
                     <th className="border border-slate-300 p-4 text-left w-[35%]">Kode Token Penuh</th>
                     <th className="border border-slate-300 p-4 text-center w-[15%]">Ceklis</th>
                     <th className="border border-slate-300 p-4 text-left w-[42%]">Catatan / Diberikan Kepada</th>
                   </tr>
                 </thead>
                 <tbody>
                   {chunk.map(([code, data]: any, i: number) => {
                     ...
                     return (
                       <tr key={code} className={data.isUsed ? "bg-emerald-50/30" : ""}>
                          <td className="border border-slate-300 p-4 text-center font-bold text-slate-500">{realIndex}</td>
                          <td className="border border-slate-300 p-4 font-mono font-black text-indigo-700 bg-indigo-50/50 text-xl tracking-widest">
                            {batch.id}-{code}
                          </td>
                          <td className="border border-slate-300 p-4 text-center">
                            {data.isUsed ? (
                              ...
                              <div className="w-6 h-6 border-[3px] border-slate-300 rounded mx-auto"></div>
                            )}
                          </td>
                          <td className="border border-slate-300 p-4 text-sm leading-relaxed text-slate-700">
                            {data.isUsed ? (
