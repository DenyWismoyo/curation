...
import { DynamicField } from '@/app/components/curation/DynamicField'; 
...
export function AdminTemplatePreview({ template }: { template: FormTemplate }) {
  ...
  const handleExportFormPDF = async () => {
    ...
    try {
      ...
      alert("Gagal mengunduh dokumen Struktur Form.");
    } finally {
  ...
  const handleExportAIPDF = async () => {
    ...
    try {
      ...
      alert("Gagal mengunduh dokumen Blueprint AI.");
    } finally {
      ...
      <div className="w-full md:w-64 bg-slate-50 border-r border-slate-200 p-6 flex flex-col gap-6 shrink-0">
        ...
        <div className="flex flex-col gap-2">
          <Button 
            ...
            className={`justify-start rounded-xl h-11 ${viewMode === 'form' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'text-slate-600 bg-white border-slate-200 hover:bg-slate-50 hover:text-indigo-600'}`}
          >
          ...
          <Button 
            ...
            className={`justify-start rounded-xl h-11 ${viewMode === 'ai' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'text-slate-600 bg-white border-slate-200 hover:bg-slate-50 hover:text-indigo-600'}`}
          >
        ...
        {viewMode === 'form' && totalSteps > 0 && (
          <div className="mt-auto pt-6 border-t border-slate-200">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 text-center">Navigasi Step</p>
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="bg-slate-900 rounded-3xl p-8 text-emerald-400 font-mono text-xs sm:text-sm leading-relaxed overflow-x-auto shadow-2xl ring-1 ring-slate-800 mb-10">
              ...
              <div className="space-y-1.5 mb-8">
                ...
                <p><span className="text-pink-400 font-bold">Target Analisis:</span> "{ai.assessmentGoal}"</p>
                <p><span className="text-pink-400 font-bold">Level Keketatan:</span> {ai.gradingStrictness}</p>
                <p><span className="text-pink-400 font-bold">Gaya Bahasa:</span> {ai.reportTone}</p>
