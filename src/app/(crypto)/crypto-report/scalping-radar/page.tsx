...
export default function ScalpingRadarPage() {
  ...
  if (authLoading || loading) {
    return <div className="flex justify-center items-center h-screen bg-slate-50 dark:bg-slate-950"><Loader2 className="animate-spin h-10 w-10 text-orange-500" /></div>;
  }
  ...
  if (!latestReport) {
    return <div className="p-8 text-center bg-slate-50 dark:bg-slate-950 min-h-screen text-slate-900 dark:text-white">Belum ada data Volatility Scanner.</div>;
  }
  ...
  return (
    ...
    >
      ...
      <div className="bg-slate-50 dark:bg-slate-950/40 backdrop-blur-md border-b border-white/5 mb-6">
        <div className="max-w-6xl mx-auto px-4 md:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
             <Button onClick={() => router.back()} variant="ghost" size="icon" className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-100 dark:bg-slate-800 rounded-full">
                 <ArrowLeft className="w-5 h-5" />
             </Button>
             <div>
                <div className="flex items-center gap-2">
                   <Zap className="w-5 h-5 text-orange-500 fill-orange-500" />
                   <h1 className="font-black text-xl text-slate-900 dark:text-white tracking-tight">Volatility Command Center</h1>
                </div>
          ...
          <div className="text-right flex flex-col items-end gap-2">
             ...
             <Select value={selectedReportId} onValueChange={setSelectedReportId}>
               <SelectTrigger className="w-[180px] sm:w-[220px] h-9 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 focus:ring-0 focus:ring-offset-0 rounded-xl">
                  <SelectValue placeholder="Pilih Waktu" />
               </SelectTrigger>
               <SelectContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 rounded-xl max-h-[300px]">
                  {reports.map(r => {
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-10 space-y-12">
        ...
        {/* HERO SECTION */}
        <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-orange-500/20 bg-slate-50 dark:bg-black">
          ...
          <div className="relative z-10 p-8 md:p-12">
             ...
             <div className="flex flex-wrap items-center gap-4">
                <div className="bg-slate-100 dark:bg-white/5 backdrop-blur-md rounded-2xl px-6 py-4 border border-white/5 flex items-center gap-4">
                   <Activity className="w-8 h-8 text-orange-400" />
                   <div>
                     <div className="text-xs text-slate-500 uppercase font-bold tracking-widest">Active Setups</div>
                     <div className="text-2xl font-black text-slate-900 dark:text-white">{scalps.length}</div>
                   </div>
                </div>
             </div>
          </div>
        ...
        <div className="space-y-6">
           {scalps.length === 0 ? (
             <div className="text-center p-12 border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
                ...
                <h3 className="text-lg font-bold text-slate-500 dark:text-slate-400">Tidak ada setup yang lolos uji risiko saat ini.</h3>
             </div>
           ) : (
             scalps.map((scalp: any, idx: number) => (
               <Card key={idx} className="bg-white/40 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800/60 backdrop-blur-md overflow-hidden transition-all duration-500 hover:border-orange-500/30 hover:bg-white/80 dark:bg-slate-900/80 hover:shadow-[0_0_30px_rgba(249,115,22,0.05)] group">
                 <div className="p-6 md:p-8 flex flex-col lg:flex-row gap-8">
                   ...
                   <div className="lg:w-1/3 flex flex-col justify-between relative">
                      ...
                      <div className="relative z-10">
                         <div className="flex items-center gap-3 mb-6">
                            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-slate-100 dark:from-slate-800 to-slate-200 dark:to-slate-900 flex items-center justify-center border border-slate-300 dark:border-slate-700 shadow-inner">
                              <span className="font-black text-lg text-slate-900 dark:text-white">{scalp.symbol.replace("USDT", "")}</span>
                            </div>
                            <div>
                               <h3 className="text-2xl font-black text-slate-900 dark:text-white">{scalp.symbol}</h3>
                         ...
                         {(() => {
                            ...
                            return (
                               <div className="grid grid-cols-2 gap-2 mb-6">
                                  {vwapDev && (
                                     <div className="bg-slate-50 dark:bg-black/30 p-2 rounded-lg border border-white/5">
                                  ...
                                  {rawData?.openInterest && (
                                     <div className="bg-slate-50 dark:bg-black/30 p-2 rounded-lg border border-white/5">
                                        ...
                                        <div className="text-xs font-bold text-slate-600 dark:text-slate-300">{(rawData.openInterest / 1000000).toFixed(2)}M</div>
                                     </div>
                                  )}
                                  {rawData?.adx?.adx && (
                                     <div className="bg-slate-50 dark:bg-black/30 p-2 rounded-lg border border-white/5">
                                        <div className="text-[10px] text-slate-500 uppercase font-bold">ADX Trend</div>
                                        <div className={`text-xs font-bold ${rawData.adx.adx > 25 ? 'text-orange-400' : 'text-slate-500 dark:text-slate-400'}`}>{rawData.adx.adx.toFixed(1)}</div>
                                     </div>
                                  )}
                                  {rawData?.fundingRate && (
                                     <div className="bg-slate-50 dark:bg-black/30 p-2 rounded-lg border border-white/5">
                         ...
                         <div className="space-y-4 mb-6">
                            <div className="bg-slate-50 dark:bg-black/40 rounded-xl p-4 border border-white/5 flex justify-between items-center">
                               <span className="text-xs text-slate-500 font-bold uppercase tracking-wider">Entry Target</span>
                               <span className="font-mono text-lg font-bold text-slate-900 dark:text-white">${scalp.entryPrice}</span>
                            </div>
                      ...
                      <div className="pt-4 border-t border-slate-200 dark:border-slate-800/50 flex justify-between items-center relative z-10">
                         <div className="text-xs text-slate-500 font-medium">Risk/Reward</div>
                         <div className="text-sm font-black text-slate-600 dark:text-slate-300 bg-slate-200 dark:bg-slate-800/50 px-3 py-1 rounded-md">{scalp.riskRewardRatio}</div>
                      </div>
                   </div>
                   ...
                   <div className="lg:w-2/3 border-t lg:border-t-0 lg:border-l border-slate-200 dark:border-slate-800/50 pt-6 lg:pt-0 lg:pl-8 flex flex-col">
                      <div className="mb-6 flex items-center gap-2">
                         ...
                         <h4 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">AI Multi-Agent Thesis</h4>
                      </div>
                      <div className="bg-slate-50 dark:bg-black/20 rounded-2xl p-6 border border-white/5 flex-1 shadow-inner">
                      ...
                      <div className="mt-6 flex items-center justify-between">
                         <div className="flex items-center gap-2 text-xs text-slate-500 font-medium">
                            ...
                            Alokasi yang disarankan: <span className="font-bold text-slate-600 dark:text-slate-300">{scalp.allocationPercentage}</span>
                         </div>
