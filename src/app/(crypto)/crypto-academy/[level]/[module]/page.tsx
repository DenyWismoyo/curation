...
export default function CryptoAcademyModulePage() {
  ...
  const [nextModuleId, setNextModuleId] = useState<string | undefined>(
    undefined
  )
  ...
  useEffect(() => {
    ...
    const fetchSiblingModules = async () => {
      try {
        const nq = await namedQuery(db, `crypto-academy-${levelName}`)
        let mods: CryptoModule[] = []
        if (nq) {
          const snap = await getDocsFromCache(nq)
          mods = snap.docs.map(
            (doc) => ({ id: doc.id, ...doc.data() }) as CryptoModule
          )
        } else {
          // Fallback
          const q = query(
  ...
  useEffect(() => {
    ...
    const fetchProgress = async () => {
      const snap = await getDoc(
        doc(db, 'userProgress', user.uid, 'modules', moduleId)
      )
    ...
    <>
      <div className="fixed top-0 left-0 right-0 z-50">
        <Progress
          ...
          className="h-1.5 rounded-none bg-secondary text-secondary-foreground"
        />
      </div>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 pb-32">
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div
            id="module-content"
            className="flex-1 min-w-0 break-words prose dark:prose-invert prose-lg max-w-none prose-headings:text-foreground prose-p:text-muted-foreground prose-a:text-indigo-600 dark:prose-a:text-indigo-400 prose-strong:text-foreground"
          >
          ...
          {!quizResult ? (
            <>
              ...
              <p className="text-muted-foreground mb-6 max-w-xl mx-auto">
                Tandai modul ini sebagai selesai untuk melanjutkan perjalanan
                belajarmu, atau kerjakan kuis evaluasi.
              </p>
