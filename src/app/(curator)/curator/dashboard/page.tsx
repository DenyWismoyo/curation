...
import { CuratorAssessmentDetail } from '@/features/curator/components/CuratorAssessmentDetail';
import { useMobileBack } from '@/hooks/useMobileBack';
import { logCuratorAuditEvent } from '@/services/b2b/b2b-curator-audit';
...
export default function CuratorDashboard() {
  ...
  return (
    <div className="min-h-screen bg-muted text-muted-foreground/50">
      <nav className="card-solid border-b border-border sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-emerald-100 text-emerald-600 dark:text-emerald-400 flex items-center justify-center rounded-xl">
                <ShieldCheck className="w-6 h-6" />
              </div>
              <div>
                <h1 className="font-black text-foreground leading-tight">Portal Kurator</h1>
                <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">
                  {selectedOrganization === ALL_ORGS ? 'Multi-Organisasi' : selectedOrganization}
                </p>
              </div>
            </div>
            <Button onClick={handleLogout} variant="ghost" className="text-muted-foreground hover:text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:bg-rose-500/10 font-bold gap-2">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          <Card className="p-6 card-solid rounded-3xl border-none ring-1 ring-border shadow-sm flex items-center gap-5">
            ...
            <div>
              ...
              <h3 className="text-3xl font-black text-foreground">{totalSubmissions}</h3>
            </div>
          </Card>

          <Card className="p-6 card-solid rounded-3xl border-none ring-1 ring-border shadow-sm flex items-center gap-5">
            <div className="w-14 h-14 bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded-2xl flex items-center justify-center shrink-0">
          ...
          <Card className="p-6 card-solid rounded-3xl border-none ring-1 ring-border shadow-sm flex items-center gap-5">
            ...
            <div>
              ...
              <h3 className="text-3xl font-black text-foreground">{validatedCount}</h3>
            </div>
          </Card>
        </div>

        <div className="card-solid rounded-3xl shadow-sm ring-1 ring-border overflow-hidden">
          <div className="p-6 border-b border-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h2 className="text-lg font-black text-foreground flex items-center gap-2">
              <Activity className="w-5 h-5 text-emerald-600 dark:text-emerald-400" /> Daftar Tugas Kurasi Lapangan
            </h2>
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
              {organizationScopes.length > 1 && (
                <select
                  ...
                  className="h-10 bg-muted text-muted-foreground border border-border rounded-xl px-3 text-sm font-semibold text-slate-700"
                >
              ...
              <div className="relative w-full sm:w-72">
                ...
                <Input
                  ...
                  className="pl-9 h-10 bg-muted text-muted-foreground border-border rounded-xl"
                />
              </div>
              ...
              <Button
                ...
                variant="outline"
                className="gap-2 font-bold card-solid text-indigo-600 dark:text-indigo-400 border-indigo-200 dark:border-indigo-500/20 hover:bg-indigo-50 dark:bg-indigo-500/10 rounded-xl h-10 shadow-sm w-full sm:w-auto"
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                  {visibleAssessments.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-muted text-muted-foreground/50 transition-colors">
                        <td className="px-6 py-5">
                          <div className="font-bold text-foreground text-base mb-0.5">{item.namaUsaha}</div>
                        ...
                        <td className="px-6 py-5 text-muted-foreground font-semibold">{item.corporateEntity || '-'}</td>
                        <td className="px-6 py-5 text-center">
                          <div className={`inline-flex items-center justify-center w-10 h-10 rounded-full font-black text-base ring-1 ${isFinalized ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 ring-emerald-200 dark:ring-emerald-500/20' : 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 ring-indigo-200 dark:ring-indigo-500/20'}`}>
                            {skorAkhir}
                          </div>
                        ...
                        <td className="px-6 py-5">
                          {isFinalized ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest bg-emerald-100 text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-200 dark:ring-emerald-500/20">
                              <CheckCircle2 className="w-3.5 h-3.5" /> Selesai
                            </span>
                          ) : isDraft ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest bg-amber-100 text-amber-700 dark:text-amber-300 ring-1 ring-amber-200 dark:ring-amber-500/20">
                              <Edit3 className="w-3.5 h-3.5" /> Draf
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest bg-secondary text-secondary-foreground text-muted-foreground ring-1 ring-border">
                        ...
                        <td className="px-6 py-5 text-center">
                          <Button
                            ...
                            className={`rounded-xl font-bold h-9 px-4 shadow-sm transition-all ${isFinalized ? 'card-solid text-slate-700 border border-border hover:bg-secondary text-secondary-foreground' : isDraft ? 'bg-amber-500 hover:bg-amber-600 text-white shadow-amber-200' : 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-indigo-200'}`}
                          >
                            {isFinalized ? (
      ...
      {isManageTagsOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="card-solid rounded-[2rem] p-6 w-full max-w-md shadow-2xl ring-1 ring-border animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-black text-xl text-foreground flex items-center gap-2">
                <Tag className="text-indigo-600 dark:text-indigo-400" /> Kelola Master Tags
              </h3>
              <button onClick={() => setIsManageTagsOpen(false)} className="text-slate-400 hover:text-rose-500 bg-secondary text-secondary-foreground hover:bg-rose-100 dark:hover:bg-rose-500/20 p-1.5 rounded-full transition-colors">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="flex gap-2 mb-6">
              <Input
                ...
                className="h-11 bg-muted text-muted-foreground rounded-xl"
              />
            ...
            <div className="bg-muted text-muted-foreground/80 p-4 rounded-2xl ring-1 ring-border min-h-[100px]">
              {isLoadingTags ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {masterTags.length === 0 ? (
                    ...
                    <span key={tag} className="flex items-center gap-1.5 card-solid ring-1 ring-border px-3 py-1.5 rounded-full text-xs font-bold text-slate-700 shadow-sm">
                      {tag}
                      <button onClick={() => handleRemoveTag(tag)} className="text-slate-400 hover:text-rose-500 p-0.5 rounded-full hover:bg-rose-50 dark:bg-rose-500/10 transition-colors">
                        <X size={14} />
                      </button>
                    </span>
                  ))}
