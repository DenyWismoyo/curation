      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center bg-white p-6 rounded-3xl ring-1 ring-slate-200 shadow-sm">
          <div>
            <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2">
              <KeyRound className="text-indigo-600" /> Manajemen Akses & Token
            </h1>
            <p className="text-slate-500 text-sm mt-1">Kelola kuota kurasi untuk peserta dan berikan akses untuk para kurator.</p>
          </div>
        ...
        <div className="flex gap-4 border-b border-slate-200 pb-2">
          <button 
            ...
            className={`px-6 py-3 rounded-xl font-bold transition-all ${activeTab === 'peserta' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-slate-500 hover:bg-slate-100'}`}
          >
            <div className="flex items-center gap-2"><Users className="w-4 h-4"/> Token Peserta (Batch)</div>
          </button>
          <button 
            onClick={() => setActiveTab('kurator')}
            className={`px-6 py-3 rounded-xl font-bold transition-all ${activeTab === 'kurator' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-slate-500 hover:bg-slate-100'}`}
          >
            <div className="flex items-center gap-2"><UserCheck className="w-4 h-4"/> Akses Kurator (Single)</div>
          </button>
        </div>
                        ...
                        <tr key={batch.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-6 py-5 text-center">
                            <div className="flex items-center justify-center gap-2">
                              <Button onClick={() => setSelectedBatch(batch)} variant="outline" className="border-slate-200 text-slate-700 hover:bg-slate-100 font-bold rounded-xl h-9 px-3 shadow-sm bg-white">
                                <Eye className="w-4 h-4 sm:mr-1.5" /> <span className="hidden sm:inline">Daftar Token</span>
                              </Button>
                              <Button onClick={() => exportTokensToCSV(batch.id, batch)} variant="outline" className="border-indigo-200 text-indigo-700 hover:bg-indigo-50 font-bold rounded-xl h-9 px-3 shadow-sm bg-white">
        ...
        {activeTab === 'kurator' && (
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <Card className="p-6 sm:p-8 bg-white rounded-3xl shadow-sm border-emerald-100 flex flex-col md:flex-row items-end gap-5">
              <div className="space-y-2 flex-1 w-full">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                  <Building2 className="w-3.5 h-3.5"/> Nama Program / Entitas Tugas
                </label>
                
                {/* PERUBAHAN DI SINI: Input diganti menjadi <select> */}
                <select 
                  value={curatorProgram} 
                  onChange={e => setCuratorProgram(e.target.value)} 
                  className="flex h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm ring-offset-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 disabled:cursor-not-allowed disabled:opacity-50" 
                >
                ...
                <p className="text-[10px] text-slate-400 font-medium">Pilih dari entitas yang sudah didaftarkan pada tab Token Peserta.</p>
              </div>
              <div className="space-y-2 w-full md:w-64">
                ...
                <Input 
                  ...
                  className="h-11 rounded-xl bg-slate-50 uppercase font-mono" 
                />
              </div>
              <Button 
                onClick={generateCuratorToken} 
                disabled={isGenerating} 
                className="w-full md:w-auto bg-emerald-600 hover:bg-emerald-700 h-11 px-8 rounded-xl font-bold shadow-lg shadow-emerald-200"
              >
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {curatorTokens.length === 0 ? (
                        ...
                        <tr key={token.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-6 py-5">
                            <span className="font-bold text-slate-800">{token.programName}</span>
                          </td>
                          <td className="px-6 py-5 text-center text-slate-500 font-medium">
                            {new Date(token.createdAt).toLocaleDateString('id-ID', {day: 'numeric', month: 'short', year: 'numeric'})}
                          </td>
                          <td className="px-6 py-5 text-center">
                            <Button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col ring-1 ring-slate-200 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              ...
              <button onClick={() => { setSelectedBatch(null); setSearchToken(''); }} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-200 text-slate-500 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-4 border-b border-slate-100 bg-white">
              <div className="relative">
                ...
                <Input value={searchToken} onChange={(e) => setSearchToken(e.target.value)} placeholder="Cari Token atau Nama Usaha pengguna..." className="pl-9 bg-slate-50 border-slate-200 rounded-xl font-medium" />
              </div>
            </div>
                    ...
                    <div key={code} className="bg-white p-4 rounded-2xl ring-1 ring-slate-200/60 shadow-sm flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between hover:ring-indigo-200 transition-all">
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2">
                          <code className="text-base font-black text-slate-900 font-mono bg-slate-100 px-2 py-0.5 rounded-lg border border-slate-200">{fullToken}</code>
                          {data.isUsed ? (
                      ...
                      {!data.isUsed ? (
                        <Button variant="secondary" size="sm" onClick={() => handleCopyToken(fullToken)} className={`shrink-0 rounded-xl font-bold h-9 ${copiedToken === fullToken ? 'bg-emerald-500 text-white hover:bg-emerald-600' : 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100'}`}>
                          {copiedToken === fullToken ? <><Check className="w-4 h-4 mr-1.5" /> Tersalin!</> : <><Copy className="w-4 h-4 mr-1.5" /> Copy Kode</>}
                        </Button>
                      ) : (
                        <div className="shrink-0 h-9 px-3 flex items-center text-xs font-bold text-slate-400 bg-slate-50 rounded-xl ring-1 ring-slate-100 cursor-not-allowed">Token Tidak Berlaku</div>
                      )}
                    </div>
                  );
                })
