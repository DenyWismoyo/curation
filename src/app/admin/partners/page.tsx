...
import { 
  Handshake, Plus, Trash2, Link as LinkIcon, Loader2, 
  CheckCircle2, Eye, EyeOff, UploadCloud, FileImage, 
  Edit3, X, Layers, UserCheck, MessageSquareQuote
} from 'lucide-react';
...
export default function AdminPartnersPage() {
  ...
  // Filter Tab State
  const [activeTab, setActiveTab] = useState<'semua' | 'powered_by' | 'mitra_strategis' | 'klien' | 'testimoni_ahli'>('semua');
  ...
  const [category, setCategory] = useState<'powered_by' | 'mitra_strategis' | 'klien' | 'testimoni_ahli'>('mitra_strategis');
  ...
  const loadPartners = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'landing_partners'), orderBy('order', 'asc'));
  ...
  const handleSavePartner = async () => {
    ...
    if (!isEditing && !logoFile) return toast.warning('File Logo/Foto wajib diunggah untuk entitas baru.');
    if (category === 'testimoni_ahli' && !message.trim()) return toast.warning('Isi pesan testimoni wajib diisi untuk pakar.');
    ...
    try {
      ...
      if (logoFile) {
        ...
        const folderName = category === 'testimoni_ahli' ? '2_expert_photos' : '1_logo';
        ...
        if (isEditing && editingStoragePath) {
          ...
          await deleteObject(oldRef).catch(err => console.warn("File lama tidak ditemukan:", err));
        }
      }
      ...
      if (!isEditing) {
        ...
        await setDoc(doc(db, 'landing_partners', partnerId), partnerData as LandingPartner);
  ...
  const handleToggleActive = async (id: string, currentStatus: boolean) => {
    try {
      await updateDoc(doc(db, 'landing_partners', id), { isActive: !currentStatus });
      setPartners(prev => prev.map(p => p.id === id ? { ...p, isActive: !currentStatus } : p));
      toast.success(`Status diubah menjadi ${!currentStatus ? 'Tayang' : 'Disembunyikan'}.`);
    } catch (error) {
      console.error(error);
  ...
  const filteredPartners = activeTab === 'semua' ? partners : partners.filter(p => p.category === activeTab);
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div>
        <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
          <Handshake className="w-8 h-8 text-indigo-600" /> Kemitraan & Ekosistem
        </h1>
        <p className="text-slate-500 mt-2 font-medium max-w-2xl text-balance">
          Kelola logo institusi mitra dan testimoni dari pakar/ahli yang mendukung ekosistem Omnifit.
        </p>
      </div>
      ...
      <Card className={`p-6 sm:p-8 bg-white rounded-3xl border-none shadow-sm flex flex-col gap-6 transition-all ${isEditing ? 'ring-2 ring-amber-400 shadow-amber-500/10' : 'ring-1 ring-slate-200'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="space-y-2">
            ...
            >
              <option value="mitra_strategis">Mitra Strategis (Logo Institusi)</option>
              <option value="powered_by">Powered By (Logo Dukungan Utama)</option>
              <option value="klien">Klien / Ekosistem (Logo Pengguna)</option>
              <option value="testimoni_ahli">Testimoni Pakar (Foto & Profil Ahli)</option>
            </select>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1">
              <UploadCloud className="w-3 h-3"/> {isEditing ? `Ganti ${isExpert ? 'Foto' : 'Logo'} (Opsional)` : `Unggah ${isExpert ? 'Foto Pakar' : 'Logo Mitra'}`}
            </label>
            <div className="relative flex items-center gap-3">
              {isEditing && existingLogoUrl && !logoFile && (
                <div className={`w-12 h-12 bg-slate-50 border border-slate-200 shrink-0 flex items-center justify-center overflow-hidden ${isExpert ? 'rounded-full' : 'rounded-xl p-1'}`}>
                  <img src={existingLogoUrl} alt="Current" className={`max-w-full max-h-full ${isExpert ? 'object-cover w-full h-full' : 'object-contain'}`} />
                </div>
              )}
        ...
        <div className="flex justify-end pt-4 border-t border-slate-100">
          ...
          >
            {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin mr-2"/> : (isEditing ? <Edit3 className="w-4 h-4 mr-2"/> : <CheckCircle2 className="w-4 h-4 mr-2"/>)}
      ...
      <Card className="bg-white rounded-3xl overflow-hidden shadow-sm ring-1 ring-slate-200 border-none flex flex-col">
        <div className="px-4 pt-4 border-b border-slate-100 flex items-center gap-2 overflow-x-auto hide-scrollbar bg-slate-50/30">
          {[
            ...
            { id: 'mitra_strategis', label: 'Mitra Strategis', icon: Handshake },
            ...
            >
              ...
              {tab.label}
              {tab.id === 'semua' && (
                <span className={`ml-1.5 px-2 py-0.5 rounded-md text-[10px] ${activeTab === tab.id ? 'bg-indigo-100 text-indigo-800' : 'bg-slate-100 text-slate-500'}`}>
                  {partners.length}
                </span>
              )}
            </button>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center text-slate-400 font-medium space-y-2">
            ...
            <p className="text-sm font-bold text-slate-700">Belum ada data di kategori ini.</p>
          </div>
        ) : (
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className={`bg-white border border-slate-200 p-1 flex items-center justify-center relative overflow-hidden ${isPakar ? 'w-14 h-14 rounded-full' : 'w-24 h-12 rounded-lg'}`}>
                          <img 
                            src={item.logoUrl} 
                            alt={item.name} 
                            className={`max-w-full max-h-full ${isPakar ? 'object-cover w-full h-full rounded-full' : 'object-contain'}`} 
                            onError={(e) => { (e.target as any).src = 'https://via.placeholder.com/150?text=Error'; }} 
                          />
                        </div>
                      </td>
                      <td className="px-6 py-4 max-w-[250px]">
                        <p className="font-bold text-slate-900 text-base truncate">{item.name}</p>
                        {isPakar && item.role && <p className="text-[11px] font-medium text-slate-500 truncate">{item.role}</p>}
                        {item.targetUrl && (
                          <a href={item.targetUrl} target="_blank" rel="noreferrer" className="text-[10px] text-indigo-500 hover:underline flex items-center gap-1 mt-0.5 truncate">
                            <LinkIcon size={10} className="shrink-0"/> {item.targetUrl}
                          </a>
                        )}
                        {isPakar && item.message && (
                          <div className="mt-2 text-xs text-slate-500 italic truncate" title={item.message}>
                            "{item.message}"
                          </div>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold border ${isPakar ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-slate-100 text-slate-700 border-slate-200'}`}>
                          {getCategoryLabel(item.category)}
                        </span>
                        <div className="flex items-center gap-2 mt-2 ml-1">
                          <span className="text-[10px] font-black text-slate-400 uppercase">Urutan Tampil:</span>
                          <span className="text-xs font-black text-indigo-700 bg-indigo-50 px-1.5 rounded">{item.order}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        ...
                        >
                          {item.isActive ? <><Eye size={12}/> Tayang</> : <><EyeOff size={12}/> Sembunyi</>}
                        </button>
                      </td>
                      <td className="px-6 py-4 text-center">
