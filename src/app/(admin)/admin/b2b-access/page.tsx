  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <Tabs defaultValue="users" className="w-full">
        <TabsList className="bg-slate-100/70 p-1.5 rounded-2xl mb-6">
          <TabsTrigger value="users" className="rounded-xl px-6 py-2.5 font-bold text-sm data-[state=active]:bg-white data-[state=active]:text-indigo-700 data-[state=active]:shadow-sm">Akses & Persona User</TabsTrigger>
          <TabsTrigger value="orgs" className="rounded-xl px-6 py-2.5 font-bold text-sm data-[state=active]:bg-white data-[state=active]:text-indigo-700 data-[state=active]:shadow-sm">Tenant & Organisasi B2B</TabsTrigger>
        </TabsList>

        <TabsContent value="users" className="space-y-6 mt-0 outline-none">
          <Card className="rounded-[1.75rem] border-none ring-1 ring-slate-200 p-8 bg-white shadow-sm space-y-8">
            <div>
              <h2 className="text-sm font-black uppercase tracking-[0.2em] text-slate-500 mb-1">
                Grant Akses User B2B
              </h2>
              <p className="text-slate-500 text-sm">Delegasikan wewenang manajemen tenant kepada user baru atau user yang sudah ada.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <label className="block">
                ...
                <Input
                  ...
                  className="mt-1.5 h-11 rounded-xl bg-slate-50 focus:bg-white"
                />
              </label>
              <label className="block">
                <span className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">
              ...
              <label className="block md:col-span-2">
                ...
                <select
                  ...
                  className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 focus:bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                >
              ...
              <div className="md:col-span-1 rounded-xl bg-indigo-50/50 ring-1 ring-indigo-100 p-3 flex items-center gap-3 text-indigo-800">
                ...
                {PERSONA_OPTIONS.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col text-left p-4 rounded-2xl border-2 transition-all ${active ? 'border-indigo-600 bg-indigo-50/50 shadow-sm' : 'border-slate-100 bg-white hover:border-slate-200'}`}
                    >
                      <div className="flex items-center justify-between w-full">
                        <span className={`text-sm font-black uppercase tracking-widest ${active ? 'text-indigo-700' : 'text-slate-700'}`}>{option.label}</span>
                        {active && <CheckCircle2 className="w-4 h-4 text-indigo-600" />}
                      </div>
                      <span className={`text-xs mt-1.5 font-medium leading-relaxed ${active ? 'text-indigo-600/80' : 'text-slate-500'}`}>{descriptions[option.value]}</span>
                    </button>
                  )
                })}
              </div>
            ...
            <div>
              ...
              {organizations.length === 0 ? (
                <div className="rounded-xl bg-amber-50 text-amber-800 ring-1 ring-amber-200 p-4 text-sm font-medium">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {organizations.map((org) => {
                    ...
                    return (
                      <label
                        key={org.id}
                        className={`flex gap-3 rounded-2xl border-2 px-4 py-3 cursor-pointer transition-all ${checked ? 'border-indigo-600 bg-indigo-50/30' : 'border-slate-100 bg-white hover:border-slate-200'}`}
                      >
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => handleOrganizationToggle(org.id)}
                          className="mt-1 shrink-0 rounded border-slate-300 text-indigo-600 focus:ring-indigo-600"
                        />
                        <div>
            ...
            <div className="rounded-2xl border-2 border-indigo-100 bg-gradient-to-br from-indigo-50/80 to-white p-5 space-y-3">
              <p className="text-[11px] font-black uppercase tracking-[0.2em] text-indigo-700 flex items-center gap-2">
              ...
              <label className="inline-flex items-center gap-2.5 text-sm font-bold text-indigo-900 mt-2 cursor-pointer">
                <input
                  ...
                  className="rounded border-indigo-300 w-4 h-4 text-indigo-600"
                />
                Buat / Perbarui Kode Akses Curator Otomatis
              </label>
              {latestCuratorToken && (
                <div className="mt-3 inline-flex rounded-xl bg-white ring-1 ring-indigo-200 px-4 py-2.5 text-xs shadow-sm">
                  <span className="font-bold text-slate-500 mr-2">Kode Baru:</span>
            ...
            {feedback && (
              <div className={`rounded-xl p-4 text-sm font-medium ${feedback.includes('berhasil') ? 'bg-emerald-50 text-emerald-800 ring-1 ring-emerald-200' : 'bg-rose-50 text-rose-800 ring-1 ring-rose-200'}`}>
                {feedback}
              </div>
            )}
          ...
          <Card className="rounded-[1.75rem] border-none ring-1 ring-slate-200 p-8 bg-white shadow-sm overflow-hidden">
            ...
            <div className="overflow-x-auto -mx-8 px-8">
              <table className="w-full min-w-[860px] text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-left text-[10px] uppercase tracking-[0.2em] text-slate-400">
                ...
                <tbody className="divide-y divide-slate-100">
                  {rows.map((row) => (
                    <tr key={row.id} className="align-middle hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-4 pr-4">
                        <div className="flex flex-wrap gap-1">
                          {(row.b2bPersonas || []).map((p) => (
                            <span key={p} className="text-[10px] uppercase font-black tracking-wider bg-indigo-50 text-indigo-700 px-2 py-1 rounded-md">{p}</span>
                          ))}
                      ...
                      <td className="py-4 text-right">
                        <Button
                          ...
                          disabled={loadingSave}
                          variant="ghost"
                          className="h-8 px-3 rounded-lg text-rose-600 hover:text-rose-700 hover:bg-rose-50 font-bold text-xs"
                        >
                          {loadingSave ? (
              ...
              {rows.length === 0 && (
                <div className="rounded-xl bg-slate-50 border border-dashed border-slate-200 p-8 text-center text-sm text-slate-500 mt-4">
        ...
        <TabsContent value="orgs" className="space-y-6 mt-0 outline-none">
          <Card className="rounded-[1.75rem] border-none ring-1 ring-slate-200 p-8 bg-white shadow-sm space-y-8">
            <div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-medium text-slate-500 mt-1">Daftarkan entitas korporat atau organisasi B2B ke dalam ekosistem Omnifit.</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <label className="block">
                ...
                <Input
                  ...
                  className="mt-1.5 h-11 rounded-xl bg-slate-50 focus:bg-white uppercase font-mono text-sm"
                />
              </label>
              <label className="block">
                <span className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">
              ...
              <label className="block">
                ...
                <select
                  ...
                  className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm focus:ring-2 focus:ring-indigo-500/20"
                >
              ...
              <label className="block md:col-span-2">
                ...
                <Input
                  ...
                  className="mt-1.5 h-11 rounded-xl bg-slate-50 focus:bg-white font-mono text-xs"
                />
              </label>
            </div>
            ...
            <label className="block">
              ...
              <textarea
                ...
                className="mt-1.5 w-full min-h-[100px] rounded-xl border border-slate-200 bg-slate-50 focus:bg-white px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none"
          ...
          {/* DAFTAR TENANT CARD */}
          <Card className="rounded-[1.75rem] border-none ring-1 ring-slate-200 p-8 bg-white shadow-sm overflow-hidden">
            <h2 className="text-sm font-black uppercase tracking-[0.2em] text-slate-800 mb-6">
              Daftar Tenant Terdaftar
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {organizations.map(org => (
                <div key={org.id} className="border-2 border-slate-100 rounded-2xl p-5 hover:border-indigo-200 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div className="p-2 bg-indigo-50 text-indigo-600 rounded-lg">
                      <Building2 className="w-4 h-4" />
                    </div>
                    <Badge variant="secondary" className={`${org.status === 'active' ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-100' : 'bg-amber-100 text-amber-700 hover:bg-amber-100'} text-[10px] uppercase font-black`}>
                      {org.status || 'pilot'}
                    </Badge>
                  </div>
                  <h3 className="font-bold text-slate-900">{org.displayName || org.name}</h3>
              ...
              {organizations.length === 0 && (
                <div className="col-span-full py-8 text-center text-slate-500 font-medium">Belum ada tenant.</div>
              )}
            </div>
          </Card>
        </TabsContent>
      ...
      <Card className="rounded-[1.75rem] border-none ring-1 ring-slate-200 p-6 bg-gradient-to-r from-slate-900 to-indigo-900 text-white">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap gap-2 shrink-0">
            <Link
              href="/assessor"
              className="inline-flex items-center h-10 px-5 rounded-xl bg-white/10 hover:bg-white/20 text-xs font-black uppercase tracking-widest transition-colors backdrop-blur-sm"
            >
              Portal Assessor
            </Link>
            <Link
