...
export function B2BInteractionModule({
  ...
  if (loading) {
    return <div className="p-4 text-sm text-slate-500 flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Memuat riwayat interaksi...</div>;
  }

  return (
    <div className="bg-white rounded-3xl ring-1 ring-slate-200 p-6 shadow-sm flex flex-col h-full">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
        ...
        <button
          ...
          className="flex items-center gap-2 h-9 px-4 rounded-xl bg-indigo-50 text-indigo-700 font-bold text-sm ring-1 ring-indigo-200 disabled:opacity-50 hover:bg-indigo-100 transition-colors"
        >
      ...
      <div className="space-y-4 mb-6">
        {logs.map((log) => (
          <div key={log.id} className="flex gap-4">
            ...
            <div className="bg-slate-50 rounded-xl p-3 ring-1 ring-slate-100 flex-1">
      ...
      <div className="bg-slate-50 rounded-2xl p-5 ring-1 ring-slate-200 mt-auto">
        <h4 className="text-xs font-black uppercase tracking-wider text-slate-700 mb-3">Add New Log</h4>
        <div className="flex gap-2 mb-3">
          <select 
            value={newType} 
            onChange={(e) => setNewType(e.target.value)}
            className="h-10 rounded-xl border border-slate-200 text-sm px-3 bg-white font-medium text-slate-700"
          >
        ...
        <textarea
          ...
          className="w-full min-h-[100px] rounded-xl border border-slate-200 text-sm p-4 mb-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
        />
