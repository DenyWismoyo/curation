...
export default function CryptoLiveTicker() {
  ...
  return (
    <div className="w-full backdrop-blur-xl card-solid/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 shadow-sm py-2.5 px-5 flex items-center justify-between lg:justify-start gap-6 overflow-hidden rounded-2xl mb-8 relative">
      ...
      <div className="flex items-center gap-2 text-muted-foreground text-muted-foreground text-xs font-black uppercase tracking-widest shrink-0 relative z-10">
      ...
      <div className="flex items-center gap-8 overflow-x-auto no-scrollbar flex-1 relative z-10 [mask-image:linear-gradient(to_right,black_90%,transparent_100%)]">
        {Object.values(tickers).map((ticker) => (
          <div key={ticker.symbol} className="flex items-center gap-2.5 shrink-0 group transition-transform hover:scale-105 cursor-default">
            <span className="font-bold text-sm text-foreground drop-shadow-sm">{ticker.symbol}</span>
            <span 
              className={`font-mono text-sm font-semibold transition-colors duration-300 drop-shadow-sm ${
                ticker.flash === "up" ? "text-emerald-400" : 
                ticker.flash === "down" ? "text-rose-400" : "text-muted-foreground text-muted-foreground"
              }`}
            >
              ${ticker.price}
            </span>
            <span className={`flex items-center text-xs font-black px-1.5 py-0.5 rounded-md transition-colors ${ticker.isUp ? "bg-emerald-100/50 text-emerald-700 dark:text-emerald-300 bg-emerald-500/10 text-emerald-400" : "bg-rose-100/50 text-rose-700 dark:text-rose-300 bg-rose-500/10 text-rose-400"}`}>
