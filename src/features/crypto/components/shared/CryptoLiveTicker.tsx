...
export default function CryptoLiveTicker() {
  ...
  return (
    <div className="w-full backdrop-blur-xl bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 shadow-sm py-2.5 px-5 flex items-center justify-between lg:justify-start gap-6 overflow-hidden rounded-2xl mb-8 relative">
      ...
      <div className="flex items-center gap-2 text-slate-500 text-slate-500 dark:text-slate-400 text-xs font-black uppercase tracking-widest shrink-0 relative z-10">
          ...
          <div key={ticker.symbol} className="flex items-center gap-2.5 shrink-0 group transition-transform hover:scale-105 cursor-default">
            ...
            <span 
              className={`font-mono text-sm font-semibold transition-colors duration-300 drop-shadow-sm ${
                ...
                ticker.flash === "down" ? "text-rose-400" : "text-slate-600 text-slate-600 dark:text-slate-300"
              }`}
            >
              ${ticker.price}
            </span>
