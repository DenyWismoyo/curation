            ...
            <div 
              ...
              className={`snap-center shrink-0 w-[100px] rounded-2xl p-4 cursor-pointer transition-all duration-300 border 
                ${isLatest 
                  ...
                  : 'card-solid text-muted-foreground dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-sm'
                }
              `}
            >
              <div className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${isLatest ? 'text-indigo-100' : 'text-slate-400 dark:text-muted-foreground'}`}>
                {dayName}
              </div>
              <div className={`text-2xl font-black mb-0.5 ${isLatest ? 'text-white' : 'text-foreground'}`}>
                {dayNum}
              </div>
              <div className={`text-xs font-semibold ${isLatest ? 'text-indigo-200' : 'text-muted-foreground dark:text-muted-foreground'}`}>
                {monthStr}
              </div>
            </div>
          );
          ...
          {selectedReport && (
            <>
              <div className="bg-indigo-600 p-6 text-white relative overflow-hidden">
                ...
                <CryptoBadge variant="info" className="mb-3 card-solid/20 border-0 text-white">Daily Outlook</CryptoBadge>
