...
import { FormTemplate, FormField, FormStep } from '@/features/assessment/types/assessment.types'
import { DynamicField } from './DynamicField'
import { ReviewAndConfirm } from '../result/ReviewAndConfirm'
...
export function DynamicWizard({
  ...
  const StepIcon =
    currentStepData?.icon && (LucideIcons as any)[currentStepData.icon]
  ...
  const handleClearForm = () => {
    if (
      window.confirm(
        'Apakah Anda yakin ingin mengosongkan semua isian form dan mengulang dari awal?'
      )
    ) {
      setFormData({})
      if (typeof window !== 'undefined') {
  ...
  const executeAdaptiveFieldInjection = async (
    targetStepData: FormStep,
    ...
    try {
      ...
      const response = await generateQuestions({
        ...
        targetMetrics: targetStepData.targetMetrics,
      ...
      toast.error('Gagal menyinkronkan pertanyaan AI. Tekan Coba Lagi.', {
        duration: 4000,
      })
  ...
  const handleNext = async () => {
    ...
    if (step < localSteps.length) {
      ...
      if (
        template.formMode === 'adaptive' &&
        nextStepData.fields &&
        nextStepData.fields.length > 0
      ) {
        nextStepData.fields = []
    ...
    // 2. MACRO-ADAPTIVE BRANCHING (Di Ujung Form)
      ...
      try {
        const functions = getFunctions(app, 'asia-southeast2')
        const evaluateBranching = httpsCallable(
          functions,
        ...
        const response = await evaluateBranching({
          ...
          maxAdaptiveSections:
            (template.aiPromptConfig as any)?.maxAdaptiveSections || 10,
        })
        const data = response.data as {
          requiresNewSection: boolean
          newStep?: FormStep
        }
        ...
        if (data.requiresNewSection && data.newStep) {
          ...
          toast.success(
            'Seksi investigasi khusus berhasil disusun berdasarkan jawaban sebelumnya.'
          )
        } else {
        ...
        toast.error('Gagal mengevaluasi seksi lanjutan. Tekan Coba Lagi.', {
          duration: 4000,
        })
        setGenerationError(true)
      } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#FAFAFA] overflow-hidden">
      ...
      <main
        ...
        className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar relative z-10 scroll-smooth"
      >
                ...
                <div className="space-y-6 sm:space-y-8 pt-2">
                  ...
                  currentStepData.fields.length === 0 ? (
                    <div className="text-center py-10 text-slate-400 font-medium italic">
                      Seksi ini belum memiliki pertanyaan. Lanjutkan ke langkah
                      berikutnya.
                    </div>
                  ) : (
                    currentStepData.fields
