...
function replaceClasses(content) {
  // text-white -> text-slate-900 dark:text-white (only if not inside badge or button)
  // bg-slate-900 -> bg-white dark:bg-slate-900
  // text-slate-300 -> text-slate-600 dark:text-slate-300
  // text-slate-400 -> text-slate-500 dark:text-slate-400
  // border-slate-800 -> border-slate-200 dark:border-slate-800
  
  // Basic replacements that are mostly safe in CryptoHub contexts:
  let c = content;
  
  c = c.replace(/(?<!dark:)bg-slate-900(?!\/)/g, 'bg-white dark:bg-slate-900');
  
  // Specific replacements for crypto-report/page.tsx elements:
  ...
  c = c.replace(/text-slate-900 dark:text-white\s+mb-1/g, 'text-slate-900 dark:text-white mb-1'); // Actually, the cards are fine with adaptive text!
  c = c.replace(/bg-indigo-500 hover:bg-indigo-600 text-slate-900 dark:text-white/g, 'bg-indigo-500 hover:bg-indigo-600 text-white');
  c = c.replace(/bg-blue-500 hover:bg-blue-600 text-slate-900 dark:text-white/g, 'bg-blue-500 hover:bg-blue-600 text-white');
